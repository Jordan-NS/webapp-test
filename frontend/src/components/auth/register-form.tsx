  ...
  <div>
    ...
    <input
      ...
      className={`appearance-none rounded-none relative block w-full px-3 py-2 border ${
        error?.field === field ? 'border-red-500' : 'border-gray-300'
      } placeholder-gray-500 text-gray-900 ${
        field === 'name' ? 'rounded-t-md' : field === 'password' ? 'rounded-b-md' : ''
...
const ErrorMessage = ({ error }: { error: RegisterError }) => (
  <div className={`p-4 rounded-lg ${
    error.field === 'email' && error.message.includes('já está cadastrado')
      ? 'bg-[#c75c5c]/10 border border-[#c75c5c]'
    ...
    <div className="text-[#c75c5c] text-sm text-center">
      ...
      {error.field === 'email' && error.message.includes('já está cadastrado') && (
        <div className="mt-2">
          <p className="text-gray-400 mb-2">Você já possui uma conta?</p>
          <a 
            href="/login" 
            className="inline-block px-4 py-2 text-sm font-medium text-white bg-[#c75c5c] hover:bg-[#4f5d73] rounded-lg transition-colors duration-300"
          >
            Ir para o login
          </a>
        </div>
